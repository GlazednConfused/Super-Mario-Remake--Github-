...
public class player_script : MonoBehaviour
{
    ...
    void PlayerRaycast()
    {
        ...
        if (rayUp != null && rayUp.collider != null && rayUp.distance < 0.2f && rayUp.collider.tag == "breakblock")
        {
        ...
        */
        ...
        RaycastHit2D rayDown = Physics2D.Raycast(transform.position, Vector2.down, 9f);//, masks[1]);
        ...
        Debug.DrawRay(transform.position, Vector2.down * 9f, Color.red);
        if (rayDown.collider != null)
        {
            Debug.Log(rayDown.collider.name);
            if (rayDown.collider.tag == "enemy")
            {
                //I recommend make the Vector2.up force number slightly less than the jump power.
                GetComponent<Rigidbody2D>().AddForce(Vector2.up * 1200);
